...
function Posts(config) {
    //POSTS_PATH = path.resolve(config.source, '_posts'),
    //DRAFTS_PATH = path.resolve(config.source, '_drafts'),
    this.config = config;
...
function create(args, path, config) {
    // Title is required
    ...
    _.each(config.post, function (value, key) {
        data.push([key + ': ' + value]);
    });
    data.push(['---']);
...
_.extend(Posts.prototype, {
    draft: function (args) {
        create(args, this.drafts, this.config);
    },
    new: function (args) {
        create(args, this.posts, this.config)
    },
